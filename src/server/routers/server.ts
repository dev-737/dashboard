...
export const serverRouter = router({
    getHubJoinServers: protectedProcedure
        ...
        .query(async ({ input, ctx }) => {
            ...
            const userGuilds = await getCachedUserGuilds(
                account.accessToken,
                userId,
            );
            ...
            const mappedServers = manageableGuilds.map((guild) => {
                ...
                const hasConnectionToCurrentHub =
                    dbServer?.connections.some((c) => c.hubId === hubId) ||
                    false;
    ...
    getServerChannels: protectedProcedure
        ...
        .query(async ({ input }) => {
            ...
            try {
                const [discordChannels, existingConnections] =
                    await Promise.all([
                        rest.get(Routes.guildChannels(serverId)) as Promise<
                            APIChannel[]
                        >,
                ...
                const processedChannels = eligibleChannels.map((channel) => {
                    ...
                    if (isThread && hasParentId(channel)) {
                        ...
                        const p = discordChannels.find(
                            (c) => c.id === parentId,
                        );
                        parentName =
                            p && "name" in p ? (p.name as string) : null;
                    } else if (hasParentId(channel)) {
                        // It's a regular channel with a category
                        categoryId = channel.parent_id;
                        const c = discordChannels.find(
                            (ch) => ch.id === categoryId,
                        );
                        categoryName =
                            c && "name" in c ? (c.name as string) : null;
                    }
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            if (!lockAcquired) {
                throw new TRPCError({
                    code: "CONFLICT",
                    message:
                        "Another process is connecting this channel. Please retry.",
                });
            }
            ...
            const connection = await db.$transaction(async (tx) => {
                ...
                if (
                    ...
                    existingServerHubConnection.id !==
                        existingChannelConnection.id
                ) {
    ...
    resolveLogConfigDetails: protectedProcedure
        ...
        .query(async ({ input, ctx }) => {
            ...
            if (account?.accessToken) {
                try {
                    const guilds = await getCachedUserGuilds(
                        account.accessToken,
                        userId,
                    );
