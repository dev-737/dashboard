import { BlockWordAction, PatternMatchType } from '@/lib/generated/prisma/client';
...
export const BlockWordActionDescriptions: Record<BlockWordAction, string> = {
  ...
  [BlockWordAction.MUTE]: 'Temporarily mutes the user for the specified duration',
...
export const BlockWordActionColors: Record<BlockWordAction, string> = {
  [BlockWordAction.BLOCK_MESSAGE]: 'bg-blue-500/20 text-blue-400 border-blue-500/30',
  [BlockWordAction.BLACKLIST]: 'bg-gray-500/20 text-gray-400 border-gray-500/30',
  [BlockWordAction.SEND_ALERT]: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30',
  [BlockWordAction.WARN]: 'bg-orange-500/20 text-orange-400 border-orange-500/30',
  [BlockWordAction.MUTE]: 'bg-purple-500/20 text-purple-400 border-purple-500/30',
...
export const MatchPatternExamples: Record<MatchPattern, string> = {
  ...
  [MatchPattern.WILDCARD]: '"bad" matches "bad", "badly", "reallybad", "badness"',
};

export function patternMatchTypeToMatchPattern(type: PatternMatchType): MatchPattern {
  switch (type) {
    case 'EXACT':
...
export function matchPatternToPatternMatchType(pattern: MatchPattern): PatternMatchType {
  switch (pattern) {
...
export function stringToPattern(pattern: string, matchType?: PatternMatchType): AntiSwearPattern {
  return {
    pattern,
...
export const RULE_TEMPLATES = {
  PROFANITY: {
    ...
    actions: [BlockWordAction.BLOCK_MESSAGE, BlockWordAction.WARN] as BlockWordAction[],
    description: 'Blocks common profanity and warns users',
  },
  HARASSMENT: {
    name: 'Anti-Harassment',
    actions: [BlockWordAction.BLOCK_MESSAGE, BlockWordAction.MUTE] as BlockWordAction[],
    description: 'Blocks harassment terms and mutes users',
  },
  SPAM: {
    name: 'Spam Prevention',
    actions: [BlockWordAction.BLOCK_MESSAGE, BlockWordAction.SEND_ALERT] as BlockWordAction[],
    description: 'Detects and blocks spam content',
  },
  ZERO_TOLERANCE: {
    name: 'Zero Tolerance',
    actions: [BlockWordAction.BLOCK_MESSAGE, BlockWordAction.BAN] as BlockWordAction[],
