...
function buildWhereClause(p: DiscoverParams): Prisma.HubWhereInput {
  const conditions: Prisma.HubWhereInput[] = [
    ...
    { connectionCount: { gte: DISCOVERY_THRESHOLDS.MIN_CONNECTIONS } },
    {
      lastActive: {
        gte: new Date(
          Date.now() -
            DISCOVERY_THRESHOLDS.MAX_INACTIVE_DAYS * 24 * 60 * 60 * 1000
        ),
      },
    },
  ];
  ...
  if (p.tags?.length)
    conditions.push({ tags: { some: { name: { in: p.tags } } } });
...
function buildOrderBy(
  sort: DiscoverSort = 'alive'
): Prisma.HubOrderByWithRelationInput[] {
  switch (sort) {
    case 'alive':
      return buildAliveSort();
    case 'active':
      return buildActiveSort();
    case 'growing':
      return buildGrowingSort();
    case 'big':
      return buildBigSort();
    case 'emerging':
      return buildEmergingSort();
    default:
      return buildAliveSort();
  }
}

function buildEmergingWhere(
  baseWhere: Prisma.HubWhereInput
