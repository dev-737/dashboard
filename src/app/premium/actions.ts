...
import { isUserBlacklisted } from '@/lib/user-blacklist';
...
export async function createCheckoutSession(
  ...
  if (await isUserBlacklisted(session.user.id)) {
    return {
      error: 'Your account has been suspended and cannot purchase premium.',
    };
  }
  ...
  try {
    ...
    if (!couponId) {
      ...
      if (activeSub) {
        ...
        const cancellingSubscription = await db.stripeSubscription.findFirst({
          where: {
...
export async function createGiftCheckoutSession(
  ...
  if (await isUserBlacklisted(session.user.id)) {
    return {
      error: 'Your account has been suspended and cannot purchase gifts.',
    };
  }
...
export async function claimGiftCode(codeId: string) {
  try {
    ...
    if (await isUserBlacklisted(session.user.id)) {
      return {
        error: 'Your account has been suspended and cannot claim gift codes.',
      };
    }
