...
export default function GiftPage() {
  ...
  const [codeStatus, setCodeStatus] = useState<GiftCodeStatus | null>(null);
  ...
  const renderContent = () => {
    ...
    if (!codeStatus || codeStatus.status === 'not_found') {
      return (
        <div className="relative z-10 flex flex-col items-center space-y-6 text-center">
          ...
          <Button asChild className="h-12 w-full rounded-2xl border border-white/10 bg-white/5 font-semibold text-white hover:bg-white/10">
    ...
    if (codeStatus.status === 'expired') {
      return (
        <div className="relative z-10 flex flex-col items-center space-y-6 text-center">
          <div className="flex h-20 w-20 items-center justify-center rounded-3xl bg-amber-500/10 text-amber-400 shadow-inner ring-1 ring-amber-500/20">
            <HugeiconsIcon
              icon={InformationCircleIcon}
              className="h-10 w-10"
            />
