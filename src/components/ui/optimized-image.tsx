...
const getValidQuality = (quality: number = 75): number => {
  ...
  return CONFIGURED_QUALITIES.reduce((closest, current) => {
    return Math.abs(current - quality) < Math.abs(closest - quality) ? current : closest;
  });
};
...
const isSlowExternalDomain = (src: string): boolean => {
  try {
    ...
    return EXTERNAL_TIMEOUT_DOMAINS.some(domain => 
      url.hostname === domain || url.hostname.endsWith('.' + domain) || url.hostname.endsWith('.ufs.sh')
    );
  } catch {
    return false;
  }
...
export function HubAvatar({ src, name, size = 56, className, ...props }: {
  src: string;
  name: string;
  size?: number;
  className?: string;
...
export function HubBanner({ src, name, className, ...props }: {
  src: string;
  name: string;
  className?: string;
...
export function ServerIcon({ src, name, size = 48, className, ...props }: {
  src: string;
  name: string;
  size?: number;
  className?: string;
