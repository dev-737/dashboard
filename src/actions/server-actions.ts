...
export async function updateServerInviteCode(serverId: string, inviteCode: string | null) {
    try {
        const session = await auth.api.getSession({
            headers: await headers(),
        });
...
export async function removeServerBlocklistEntry(serverId: string, blocklistId: string) {
    try {
        const session = await auth.api.getSession({
            headers: await headers(),
        });
